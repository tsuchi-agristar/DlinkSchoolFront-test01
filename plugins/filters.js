...
Vue.filter('view', function (value, code) {
	if(!code) {
		return
	}
	let data = _.find(CONST[code], (dt) => {
...
Vue.filter("formatTzToAsiaTokyo", function (value, format) {
	if(!value) {
		return
	}
	if(!format) {
	...
	// return moment(value).add(9, "hour").format(format); バックエンド側をJSTにした為不要となった
